...
export default function SteamReturn({user, accounts}) {
    ...
    let { trigger, data, error } = useSWRMutation(`/api/getGmodstorePurchases?steamID={steamID}&userID=${userID}`, fetcher)
    if (data) {
        if (data.status === 404) {
    ...
    return (
        <>
            <div>
                <Logo/>
                <h1 className="text-2xl font-semibold text-white">Hey {user.username}</h1>
                <h2 className="text-xl font-semibold text-white">Please select the correct Steam account.</h2>
                <p className="text-xs text-white">If you don't see your account, please make sure its linked to your Discord.</p>
                    ...
                    <div className="backdrop-blur bg-white/30 rounded-lg">
                        <div className="grid grid-cols-1 gap-1 p-1">
                            {accounts.map((account, index) => {
                                return (
                                    <Account key={index} account={account} onClick={onClick} canClick={canClick}/>
                                )
                            })}
                        </div>
                    </div>
